...
(function($env){
    ...
    //For Java the window.timer is created using the java.lang.Thread in combination
    //with the java.lang.Runnable
    $env.timer = function(fn, time, num){
        var running = true;
    ...
    $env.connection = function(xhr, responseHandler, data){
        ...
        if(connection){
            ...
            xhr.responseText = new java.lang.String(baos.toByteArray(),0, total)+"";
...
__extend__(DOMDocument.prototype, {	
    ...
    load: function(url){
        ...
        xhr.onreadystatechange = function(){
        	...
        	$info("Successfully loaded document at "+url);
        	if($env.fireLoad){
...
__extend__(HTMLButtonElement.prototype, {
    ...
    get type(){
        return this.getAttribute('type') || 'input';
    },
    set type(value){
...
__extend__(HTMLSelectElement.prototype, {
    get type(){
        return this.getAttribute('type') || "select";
    },
    get selectedIndex(){
    ...
    get value(){
        return this.getAttribute('value')||this.options[this.selectedIndex].value;
    },
    set value(value) {
        var options = this.options,
            i, index;
...
var Event = function(options){
  ...
  return __extend__(this,{
    ...
    initMouseEvent: function(type,bubbles,cancelable){
      $type=type?type:$type;
      $bubbles=bubbles?bubbles:$bubbles;
      $cancelable=cancelable?cancelable:$cancelable;
    },
...
window.setInterval = sync(function(fn, time){
	...
	if(time===0){
    	...
    	$timers[num] = new $env.timer(fn, time, num);
    	$timers[num].start();
	}
	return num;
});

window.clearInterval = window.clearTimeout = sync(function(num){
	if ( $timers[num] ) {
...
$w.dispatchEvent = function(event){
    ...
    //calculate parents
    var current = event.target, bubbles = [current];
    
    while(current = current.parentNode) bubbles.push(current);
    
    //now dispatch events on each bubble
    for(var i  = 0; i < bubbles.length; i++){
    	var bubble = bubbles[i];
    	event.currentTarget = bubble;
    	if ( event.type ) {
	        if ( bubble.uuid && $events[bubble.uuid][event.type] ) {
	            
	            $events[bubble.uuid][event.type].forEach(function(fn){
	                $debug('calling event handler '+fn+' on target '+bubble);
	                fn.call( bubble, event );
	            });
	        }
	    
	        if (bubble["on" + event.type]) {
	            $debug('calling event handler '+event.type+' on target '+bubble);
	            bubble["on" + event.type].call(bubble, event);
	        }
	    }
    }
    /*
